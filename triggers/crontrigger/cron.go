...
func init() {
	triggers.RegisterTriggerHandler(&CronTrigger{})
}

// CronTrigger implements the jobs.Trigger interface.
...
var _ triggers.Trigger = (*CronTrigger)(nil)

// New returns a new CronTrigger using the UTC location.
func New(expr string) (*CronTrigger, error) {
	return NewWithLoc(expr, time.UTC)
}

// MustNewCronTrigger returns a new CronTrigger using the UTC location, will panic on invalid expression.
func MustNewCronTrigger(expr string) *CronTrigger {
	trig, err := NewWithLoc(expr, time.UTC)
	if err != nil {
		panic(err)
	}
	return trig
...
func (t CronTrigger) Expr() string {
	return t.expression
}

func (t CronTrigger) Loc() *time.Location {
	return t.location
}

type persistetedCronTrigger struct {
	Expression string
	Location   string
}

func (ct *CronTrigger) Serialize() (string, error) {
	data, err := json.Marshal(persistetedCronTrigger{
		Expression: ct.expression,
		Location:   ct.location.String(),
	})
	return string(data), errors.Wrap(err)
}

func (ct *CronTrigger) Deserialize(data string) (triggers.Trigger, error) {
	trig := persistetedCronTrigger{}
	err := json.Unmarshal([]byte(data), &trig)
	if err != nil {
		return nil, errors.Wrap(err)
	}
	loc, err := time.LoadLocation(trig.Location)
	if err != nil {
		return nil, errors.Wrap(err)
	}
	return NewWithLoc(trig.Expression, loc)
}
...
// NewWithLoc returns a new CronTrigger with the given time.Location.
func NewWithLoc(expr string, location *time.Location) (*CronTrigger, error) {
...
func (ct *CronTrigger) NextFireTime(prev time.Time) (time.Time, error) {
	...
	prev = prev.In(ct.location)
	nextTime, err := parser.nextTime(prev, ct.fields)
...
func (parser *CronExpressionParser) nextTime(prev time.Time, fields []*CronField) (nextTime time.Time, err error) {
	...
	nextTime, err = time.ParseInLocation(writeDateLayout, nstr, prev.Location())
	return
}
